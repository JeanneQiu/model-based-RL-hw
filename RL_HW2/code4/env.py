...
class Make_Env(object):
    ...
    def R(self, s, a, s_):
        ...
        #     r = 9
        if (s[2] == 1 or not self.use_key) and np.array_equal(s_[:2], self.pos_door_init):
            r = 99  # 到达门的位置并且拿到了钥匙（或不需要钥匙）
        elif (s[2] == 0 and self.use_key) and np.array_equal(s_[:2], self.pos_key_init):
            r = 9  # 到达钥匙的位置，且还需要钥匙
        else:
            r = -1
        return r
